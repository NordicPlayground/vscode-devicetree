...
export class TreeInfoItem {
    ...
    get treeItem(): vscode.TreeItem {
        ...
        item.id = ['devicetree', 'ctx', this.ctx.name, 'item', this.id].join('.');
        if (this.icon) {
            item.iconPath = iconPath(this.icon);
        }
        ...
        if (this.loc) {
            item.command = {
                ...
                arguments: [
                    this.loc.uri,
                    { selection: this.loc.range } as vscode.TextDocumentShowOptions,
                ],
            };
        } else if (this.path) {
            item.command = {
                ...
                title: 'Show',
                arguments: [this.path, this.ctx.files.pop().uri],
            };
        }
...
export class DTSTreeView implements vscode.TreeDataProvider<DTSTreeItem> {
    ...
    activate(ctx: vscode.ExtensionContext) {
        ...
        this.treeView = vscode.window.createTreeView('trond-snekvik.devicetree.ctx', {
        ...
        Object.entries({
            ...
            if (ctx.board.info[field]) {
                ...
                if (Array.isArray(ctx.board.info[field])) {
                    (<string[]>ctx.board.info[field]).forEach((i) =>
                        item.addChild(new TreeInfoItem(ctx, i))
                    );
                } else {
    ...
    private flashOverview(ctx: DTSCtx) {
        ...
        ctx.nodeArray()
            ...
            .forEach((n, _, all) => {
                ...
                let offset = 0;
                n.children()
                    .filter((c) => c.regs()?.[0]?.addrs.length === 1)
                    .sort(
                        (a, b) =>
                            (a.regs()[0].addrs[0]?.val ?? 0) - (b.regs()[0].addrs[0]?.val ?? 0)
                    )
                    .forEach((c) => {
                        const reg = c.regs();
                        ...
                        if (start > offset) {
                            parent.addChild(
                                new TreeInfoItem(
                                    ctx,
                                    `Free space @ 0x${offset.toString(16)}`,
                                    undefined,
                                    sizeString(start - offset)
                                )
                            );
                        }

                        const partition = new TreeInfoItem(
                            ctx,
                            (c.property('label')?.value?.[0]?.val as string) ?? c.uniqueName
                        );
                        partition.description = sizeString(size);
                        if (start < offset) {
                            partition.description += ` - ${sizeString(offset - start)} overlap!`;
                        }
                        partition.tooltip = `0x${start.toString(16)} - 0x${(
                            start +
                            size -
                            1
                        ).toString(16)}`;
        ...
        if (!flash.children.length) {
            ctx.nodeArray()
                .filter((n) => n.type?.is('soc-nv-flash'))
                .forEach((n, _, all) => {
                    let parent = flash;
                    if (all.length > 1) {
                    ...
                    n.regs()
                        ?.filter((reg) => reg.addrs.length === 1 && reg.sizes.length === 1)
                        .forEach((reg, i, areas) => {
                            let area = parent;
                            if (areas.length > 1) {
    ...
    private interruptOverview(ctx: DTSCtx) {
        ...
        const controllerItems = controllers.map((n) => ({
            item: new TreeInfoItem(ctx, n.uniqueName),
            children: new Array<{ node: Node; interrupts: Property }>(),
        }));
        nodes
            .filter((n) => n.property('interrupts'))
            .forEach((n) => {
        ...
        controllerItems
            ...
            .forEach((controller, i) => {
                ...
                controller.children
                    .sort((a, b) => a.interrupts.array?.[0] - b.interrupts.array?.[0])
                    .forEach((child) => {
                        ...
                        childIrqs?.forEach((cellValues, i, all) => {
                            ...
                            cells?.forEach((cell, i) =>
                                irq.addChild(
                                    new TreeInfoItem(
                                        ctx,
                                        cell.replace(/^\w/, (letter) => letter.toUpperCase()) + ':',
                                        undefined,
                                        cellValues?.[i]?.toString() ?? 'N/A'
                                    )
                                )
                            );
            ...
            .forEach((node) => {
                ...
                node.uniqueProperties()
                    .filter(
                        (prop) =>
                            prop.value.length > 0 &&
                            busProps.some((regex) => prop.name.match(regex))
                    )
                    .forEach((prop) => {
                        const infoItem = new TreeInfoItem(
                            ctx,
                            prop.name.replace(/-/g, ' ') + ':',
                            undefined,
                            prop.value.map((v) => v.toString(true)).join(', ')
                        );
                    ...
                    if (child.address !== undefined) {
                        ...
                        if (node.type?.bus === 'spi') {
                            ...
                            if (cs) {
                                ...
                                csEntry.description = `${cs.target.toString(true)} ${cs.cells
    ...
    private ioChannelOverview(type: 'ADC' | 'DAC', ctx: DTSCtx) {
        ...
        nodes
            .filter((node) => node.type?.is(type.toLowerCase() + '-controller'))
            .forEach((node) => {
                ...
                nodes
                    .filter((n) =>
                        n
                            .property('io-channels')
                            ?.entries?.some(
                                (entry) => entry.target instanceof PHandle && entry.target.is(node)
                            )
                    )
                    .flatMap((usr) => {
                        ...
                        return usr
                            .property('io-channels')
                            .entries.filter((c) => c.target.is(node))
                            .map((channel, i, all) => ({
                                node: usr,
                                idx: channel.cells[0]?.val ?? -1,
                                name: names[i] ?? (all.length > 1 && i.toString()),
                            }));
                    })
                    .sort((a, b) => a.idx - b.idx)
                    .forEach((channel) => {
                        const entry = new TreeInfoItem(
                            ctx,
                            `Channel ${channel.idx}`,
                            undefined,
                            channel.node.uniqueName + (channel.name ? ` • ${channel.name}` : '')
                        );
                ...
                if (!controller.children.length) {
                    controller.addChild(
                        new TreeInfoItem(ctx, '', undefined, 'No channels in use.')
                    );
                }
    ...
    private clockOverview(ctx: DTSCtx) {
        ...
        nodes
            .filter((node) => node.type?.is('clock-controller'))
            .forEach((node) => {
                ...
                nodes.forEach((user) => {
                    ...
                    entries?.forEach((e) => {
                        ...
                        cells?.forEach((c, i) => {
                            if (i < e.cells.length) {
                                userEntry.addChild(
                                    new TreeInfoItem(ctx, c, undefined, e.cells[i].toString(true))
                                );
                            }
                        });
    ...
    getOverviewTree(ctx: DTSCtx): vscode.ProviderResult<DTSTreeItem[]> {
        const details = this.details(ctx);
