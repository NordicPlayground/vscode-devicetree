...
suite('Parser test suite', () => {
    ...
    test('Preprocessor', async () => {
        ...
        if (BAKE_OUTPUT) {
            fs.writeFileSync(
                extensionDevelopmentPath + '/src/test/output.h',
                result.lines.map((l) => l.text).join('\n')
            );
        }

        const expected = fs
            .readFileSync(extensionDevelopmentPath + '/src/test/output.h', 'utf-8')
            .split(/\r?\n/g);
        ...
        result = await preprocess(
            doc,
            toDefines([new Define('TEST_DIAGS', ''), new Define('TEST_VALID_DIRECTIVES', '')]),
            [],
            diags
        );
        ...
        result = await preprocess(
            doc,
            toDefines([new Define('TEST_DIAGS', ''), new Define('TEST_INVALID_DIRECTIVES', '')]),
            [],
            diags
        );
    });
